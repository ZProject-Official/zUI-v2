    ...
    for (let i = 0; i < text.length; i++) {
      if (text[i] === '~') {
        ...
        if (isDefined(textColor[INFO])) {
          ...
          finalText += everColoring ? `</span><span style="color: ${currentColor}">` : `<span style="color: ${currentColor}">`;
          everColoring = true;
        } else if (isDefined(fontsModifier[INFO])) {
          currentColor = fontsModifier[INFO];
          finalText += everColoring ? `</span><span style="${currentColor}">` : `<span style="${currentColor}">`;
          everColoring = true;
        } else if (isColor(INFO)) {
          currentColor = INFO;
          finalText += everColoring ? `</span><span style="color: ${INFO}">` : `<span style="color: ${INFO}">`;
          everColoring = true;
        }
